...
function IndexPage() {
  ...
  const { loading, error, educations } = educationList;
    ...
    <div class="pb-1 text-center text-gray-700 bg-black bg-cover">
      <div class="container relative max-w-2xl px-5 pt-12 mx-auto sm:py-12 lg:px-0">
        <h2 class="mb-10 text-4xl font-extrabold leading-10 tracking-tight text-left text-gray-50 sm:text-5xl sm:leading-none md:text-6xl sm:text-center">
          Your <span class="inline-block text-pink-400">Educations</span>
        </h2>
        ...
        <div class="max-w-xl px-4 mb-10 mx-auto sm:px-6 lg:max-w-screen-xl lg:px-8">
          <div class="lg:grid  lg:grid-cols-2 lg:gap-10">
            {educations &&
              educations.map((item) => {
                return (
                  <div class="flex mb-12 flex-col overflow-hidden transition duration-500 ease-in-out transform bg-gradient-to-r from-light-blue-400 to-cyan-300  rounded-lg shadow-2xl hover:scale-105">
                    <img class="h-56 rounded-t-lg  object-cover" alt="logo " src={item.instituteLogo} />
                    <div class="px-6 pt-8 mb-2 text-xl text-red-50 font-bold">
                      <span>{item.jobTitle}</span>
                    </div>
                    <div class="px-6  text-lg text-red-50 ">
                      <a href={item.website}>{item.institute}</a>
                    </div>
                    <div class="px-6  mb-2 text-lg text-red-50 ">
                      <a href={item.website}>{item.city}</a>
                    </div>
                    <div class="px-2 pt-2 text-blue-50">
                      <small>
                    ...
                    <div class="px-2 pt-2 text-blue-50">
                      ...
                      <div class="overflow-hidden h-20 ...">{item.basicinfo}</div>
                    </div>
                  </div>
                );
              })}
